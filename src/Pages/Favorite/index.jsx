...
import { Ul, Title, Bad } from "./styles";
...
export function Favorite() {
    ...
    {
        let filter = filmes.filter( (filme) => { 
            return (filme.id !==  id)
        } );

        setFilmes(filter);
        localStorage.setItem('listofmoves:', JSON.stringify(filter));
    ...
    return (
        <div>
            ...
            { filmes.length === 0 && <Bad>Você não possui nenhum filme salvo :{'('} </Bad> }
                            ...
                            <li key={item.id}>
                                ...
                                <div>
                                    ...
                                    <button onClick={ () => handleDelete(item.id) }>
                                        <svg x="0px" y="0px" viewBox="0 0 59 59" space="preserve">
